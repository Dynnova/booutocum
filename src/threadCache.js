...
function initThreadCache() {
  ...
  // Migrasi kolom untuk DB lama — harus sebelum query lain
...
// Generate short ID format: xxxx-xxxx
...
// Hapus semua thread yang expired (untuk cleanup job)
function deleteExpiredThreads() {
  const db = getDb();
  initThreadCache();
  const cutoff = new Date(Date.now() - TTL_MS).toISOString();
  const expired = db.prepare(`
    SELECT * FROM thread_cache WHERE last_accessed < ?
  `).all(cutoff);
